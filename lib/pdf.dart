...
Future<void> _createPDF(String _name, DateTime _dob, String _gender,
  ...
  await FirebaseFirestore.instance
      .doc("Users/MBTnV4LCoRSvjnyCrFZppwx55kH3")
      .collection("Vaccinations")
      .doc("Pfizer")
      .get()
      .then((value2) async {
...
//Add rows to grid
    ...
    row1.cells[1].value = value2["latest"];
...
//Add rows to grid
    ...
    // row11.cells[0].value = value2["latest"];
    // row11.cells[1].value = '12/12/2022';
    // row11.cells[2].value = 'Pfizer';
    // row11.cells[3].value = "_doseNumber";
    // row11.cells[4].value = 'Lot number';
    // row11.cells[5].value = 'Location';
    ...
    int.parse(value2["latest"]) > 1
        ? row13!.cells[0].value = _dose_number1
        : null;
    int.parse(value2["latest"]) > 1
        ? row13!.cells[1].value = '12/12/2022'
        : null;
    int.parse(value2["latest"]) > 1 ? row13!.cells[2].value = _name : null;
    int.parse(value2["latest"]) > 1
        ? row13!.cells[3].value = "_doseNumber"
        : null;
    int.parse(value2["latest"]) > 1
        ? row13!.cells[4].value = 'Lot number'
        : null;
    int.parse(value2["latest"]) > 1 ? row13!.cells[5].value = 'Location' : null;
    PdfGridRow? row14 =
        int.parse(value2["latest"]) > 2 ? grid2.rows.add() : null;
    int.parse(value2["latest"]) > 2
        ? row14!.cells[0].value = "_dose_number100"
        : null;
    int.parse(value2["latest"]) > 2
        ? row14!.cells[1].value = '12/12/2022'
        : null;
    int.parse(value2["latest"]) > 2 ? row14!.cells[2].value = 'Pfizer' : null;
    int.parse(value2["latest"]) > 2
        ? row14!.cells[3].value = "_doseNumber"
        : null;
    int.parse(value2["latest"]) > 2
        ? row14!.cells[4].value = 'Lot number'
        : null;
    int.parse(value2["latest"]) > 2 ? row14!.cells[5].value = 'Location' : null;
